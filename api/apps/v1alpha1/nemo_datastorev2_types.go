...
func (n *NemoDatastoreV2) GetInitDirContainerEnv() []corev1.EnvVar {
	// add standard env required for NIM service
	envVars := []corev1.EnvVar{
...
func (n *NemoDatastoreV2) GetInitAppINIContainerEnv() []corev1.EnvVar {
	// add standard env required for NIM service
	envVars := []corev1.EnvVar{
...
func (n *NemoDatastoreV2) GetConfigureDatastoreContainerEnv() []corev1.EnvVar {
	// add standard env required for NIM service
	envVars := []corev1.EnvVar{
		/*
					- name: GITEA_APP_INI
...
func (n *NemoDatastoreV2) GetVolumeMountsInitAppIniContainer() []corev1.VolumeMount {
	/*
					- mountPath: /usr/sbin
		      name: config
		    - mountPath: /tmp
...
func (n *NemoDatastoreV2) GetVolumeMountsInitDirContainer() []corev1.VolumeMount {
	/*
			- mountPath: /usr/sbin
		      name: init
		    - mountPath: /tmp
	...
	return []corev1.Container{
		{
			...
			Args: []string{
				"/usr/sbin/init/init_directory_structure.sh && /usr/sbin/config_environment.sh && /usr/sbin/configure_gitea.sh",
			},
