...
type NimServiceMultiNodeConfig struct {
	...
	// +kubebuilder:default:=1
	// GPUPerWorker specifies the number of GPUs for each worker. In most cases, this should match `resources.limits.nvidia.com/gpu`.
	GPUPerWorker int `json:"gpusPerWorker,omitempty"`
...
type MultiNodeMPIConfig struct {
	// CustomMPIStartScript specifies the custom MPI start script to use.
	// A default start script will be used if this is not specified.
	CustomMPIStartScript string `json:"customMPIStartScript,omitempty"`
...
func (n *NIMService) GetLWSLeaderEnv() []corev1.EnvVar {
	...
	env = append(env,
		...
		corev1.EnvVar{
			Name:  "GPUS_PER_NODE",
			Value: fmt.Sprintf("%d", n.Spec.MultiNode.GPUPerWorker),
		},
		corev1.EnvVar{
...
func (n *NIMService) GetMPIScriptConfigParams() *rendertypes.ConfigMapParams {
	...
	script := DefaultMPIStartScript
	if n.Spec.MultiNode.MPI != nil && n.Spec.MultiNode.MPI.CustomMPIStartScript != "" {
		script = n.Spec.MultiNode.MPI.CustomMPIStartScript
	}
