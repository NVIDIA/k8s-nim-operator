...
require (
	...
	github.com/NVIDIA/k8s-test-infra v0.3.0
...
// Keep Kubernetes below v0.36 so helm/kubectl still resolve
// k8s.io/api/scheduling/v1alpha1 (removed in k8s.io/api v0.36+).
// dra-driver-nvidia-gpu v0.4.1 would otherwise MVS-select k8s.io/api@v0.36.
//
// k8s.io/api is pinned to v0.34.5 because kserve's forked PodSpec is a copy of
// the core/v1 PodSpec without WorkloadRef (added in k8s 1.35 / k8s.io/api v0.35).
// Using a newer api package breaks the PodSpec conversion in kserve.
replace (
