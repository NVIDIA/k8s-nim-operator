...
func (r *DefaultAppReconciler) reconcileNemoGuardrail(ctx context.Context, NemoGuardrail *appsv1alpha1.NemoGuardrail) (ctrl.Result, error) {
	logger := log.FromContext(ctx)
	var err error
	defer func() {
		if err != nil {
			r.GetEventRecorder().Eventf(NemoGuardrail, corev1.EventTypeWarning, conditions.Failed,
				"NemoGuardrail %s failed, msg: %s", NemoGuardrail.GetName(), err.Error())
		}
	}()
