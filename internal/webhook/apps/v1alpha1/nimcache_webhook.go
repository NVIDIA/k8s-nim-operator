...
func (v *NIMCacheCustomValidator) ValidateCreate(_ context.Context, obj runtime.Object) (admission.Warnings, error) {
	...
	// Evaluate NIMSource configuration
	if err := validateNIMSourceConfiguration(&nimcache.Spec.Source); err != nil {
		return nil, err // propagate the error
	}

	// Evaluate Storage configuration
	if err := validateNIMCacheStorageConfiguration(&nimcache.Spec.Storage); err != nil {
		return nil, err
	}

	// Evaluate Proxy configuration
	if err := validateProxyConfiguration(nimcache.Spec.Proxy); err != nil {
		return nil, err
	}
