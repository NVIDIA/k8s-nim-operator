...
// TestValidateDRAResourcesConfiguration covers DRA resource validation rules.
func TestValidateDRAResourcesConfiguration(t *testing.T) {
	fldPath := field.NewPath("spec").Child("draResources")
	...
	tests := []struct {
		...
		{
			name: "multi replicas with invalid dra (name only)",
			modify: func(ns *appsv1alpha1.NIMService) {
				ns.Spec.Replicas = 2
				name := "rc1"
				ns.Spec.DRAResources = []appsv1alpha1.DRAResource{{ResourceClaimName: &name}}
			},
			wantErrs: 2,
		},
		{
			name: "multi replicas valid dra template",
			modify: func(ns *appsv1alpha1.NIMService) {
				ns.Spec.Replicas = 3
				tmpl := "tmpl1"
				ns.Spec.DRAResources = []appsv1alpha1.DRAResource{{ResourceClaimTemplateName: &tmpl}}
			},
			wantErrs: 0,
		},
		{
			name: "autoscaling enabled invalid dra name",
			modify: func(ns *appsv1alpha1.NIMService) {
				enabled := true
		...
		{
			name: "single replica duplicate names",
			modify: func(ns *appsv1alpha1.NIMService) {
				name1 := "dup"
				name2 := "dup"
				ns.Spec.DRAResources = []appsv1alpha1.DRAResource{{ResourceClaimName: &name1}, {ResourceClaimName: &name2}}
			},
			wantErrs: 1,
		},
		{
			name: "single replica missing name",
			modify: func(ns *appsv1alpha1.NIMService) {
		...
		{
			name: "single replica valid distinct names",
			modify: func(ns *appsv1alpha1.NIMService) {
				name1 := "rc1"
				name2 := "rc2"
				ns.Spec.DRAResources = []appsv1alpha1.DRAResource{{ResourceClaimName: &name1}, {ResourceClaimName: &name2}}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			ns := baseNIMService()
			// ensure Scale struct initialized
			ns.Spec.Scale.Enabled = ptr.To(false)
			tc.modify(ns)
			errs := validateDRAResourcesConfiguration(&ns.Spec, fldPath)
			if got := len(errs); got != tc.wantErrs {
				t.Logf("Validation errors:")
				for i, err := range errs {
					t.Logf("  %d: %s", i+1, err.Error())
...
// TestValidateDRAResourcesVersionCompatibility table-driven.
func TestValidateDRAResourcesVersionCompatibility(t *testing.T) {
	fld := field.NewPath("spec").Child("draResources")
	name := "rc1"
	...
	cases := []struct {
		...
		{"invalid version string", "invalid", 1},
		{"unsupported old version", "v1.32.0", 1},
		{"supported version", "v1.33.1", 0},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			ns := baseNIMService()
			ns.Spec.DRAResources = []appsv1alpha1.DRAResource{{ResourceClaimName: &name}}
			errs := validateDRAResourcesVersionCompatibility(tc.version, fld)
			if got := len(errs); got != tc.wantErrs {
				t.Logf("Validation errors:")
				for i, err := range errs {
					t.Logf("  %d: %s", i+1, err.Error())
